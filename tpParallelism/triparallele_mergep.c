...
//        printf("%d \n", valToComp);
    ...
    int main(int argc, char**argv)
    {
        ...
        int* firstTable = malloc(n/2 * sizeof(*firstTable));
        int* secondTable = malloc(n/2 * sizeof(*secondTable));

        firstTable = merge(data, data+n/4, data+n/2, cmp);
        secondTable = merge(data + n/2, data + 3*n/4, data + n, cmp);
        int valToComp = firstTable[n/4-3];
        printf("%d \n", valToComp);
